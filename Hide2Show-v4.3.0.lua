...
function ConditionalStatementInit(properties, property, settings_)
	...
	-- 
	local target_input = obs.obs_properties_get(properties, "conditional_setup_group_target_input")
	...
	obs.obs_property_set_visible(target_input,false)
	...
	if st == "cda" then
		...
		for _, n in pairs(cda_opt_list) do
...
function ConfigureSetup(properties, property, settings_)
	...
	obs.obs_data_set_string(settings_,"conditional_setup_group_target_input","")
	HIDE_ALL_PRSET_LS(properties, true); -- hide all prset;
	...
	-- [[ target ]]
		...
		if not target_list then
			...
			target_input = obs.obs_properties_add_text(configureOp, "conditional_setup_group_target_input","Enter text(*)",obs.OBS_TEXT_DEFAULT)
			obs.obs_property_set_visible(target_input, false)
			obs.obs_property_set_modified_callback(target_list, function(pr, pr2, setting)
				local target_value = obs.obs_data_get_string(setting, "conditional_setup_group_target_list")
				obs.obs_property_set_visible(target_input, false)
				if target_value == "<all_like>" then
					obs.obs_property_set_visible(target_input, true)
				end
				return true
			end)
		else
	...
	-- [[ Execute Button ]]
		...
		if not execute_btn then
			obs.obs_properties_add_button(configureOp, "conditional_setup_group_execute_btn", "Execute", function(properties, property)
				...
				local stm_tar_input = obs.obs_data_get_string(settings_,"conditional_setup_group_target_input")
				local source_target = obs.obs_data_get_string(settings_, "source_list")
				obs.obs_property_set_visible(warn_label, false)
				...
				elseif stm_v == "cdwn" and (stm_tar_v == "" or stm_tar_v  == "<none>" or stm_tar_v == nil) then
					...
					obs.obs_property_set_description(warn_label, "(Warning) select all the required options!")
					return true
				elseif stm_v == "cda" and (stm_tar_v == "" or stm_tar_v  == "<none>" or stm_tar_v == nil) then
					obs.obs_property_set_visible(warn_label, true)
					obs.obs_property_set_description(warn_label, "(Warning) select all the required options!")
					return true
				end
				if stm_v == "cda" then
					if stm_tar_input == nil or strTrim(stm_tar_input) == "" then -- Warning
						obs.obs_property_set_visible(warn_label,  true)
						obs.obs_property_set_description(warn_label, "(Warning) text/word is required for {all_like, rand_like}")
						return true
					end
					stm_tar_v = rplr_char(stm_tar_v, "<>") .. "_" .. stm_tar_input
					obs.obs_property_set_visible(warn_label,  false)
				end
				if source_target == "<all_like>" or source_target == "<rand_like>" then
...
function setSourceVisible(source_name, enable)
	...
	local function init_set(name)
		local __item = __GET_SCENE_ITEM__(name)
		if __item ~= nil then
			obs.obs_sceneitem_set_visible(__item.item, enable)
			__item.release()
		end
	end
	init_set(source_name)
    return true
end
...
function filter_action(tar)
	local filter = {}
	-- [[ ALL OR ALL_LIKE OR RAND OR RAND_LIKE ]]
	if tar == "all" or (APP_ACTIONS:IsLikeAction(tar) and APP_ACTIONS:CmdTar(tar) == "all") then -- {ALL, ALL_LIKE}
		if tar == "all" then -- hide all source;
			for _, n in ipairs(get_all_current_sources_from_scene()) do
				table.insert(filter, n)
			end
		elseif APP_ACTIONS:IsLikeAction(tar) then
			local tN = APP_ACTIONS:GetLikeAction(tar);
			for _, n in ipairs(get_all_current_sources_from_scene()) do
	...
	elseif tar == "rand" or (APP_ACTIONS:IsLikeAction(tar) and APP_ACTIONS:CmdTar(tar) == "rand") then -- {RAND, RAND_LIKE}
		if tar == "rand" then
			local sources = get_all_current_sources_from_scene();
			if #sources > 0 then
				local sn = sources[math.random(1, #sources)]
				table.insert(filter, sn)
			end
				...
				if #sources > 0 then
					while true do
						...
						if APP_ACTIONS:LikeActionMatch(tN, objectName) then
							table.insert(filter, objectName)
...
function init()
	...
	-- [[ Init operation ]]
		...
		local function get_do(act, def)
			if act == "hide" or act == "show" then
				local status = false;if act == "show" then
					status = true
				end
				return function(n)
					if n == nil then
					...
					local f = filter_action(n);
					for _, source_name in pairs(f) do
						if source_name ~= cdwn_target and source_name ~= __CURRENT_SOURCE_NAME__ then setSourceVisible(source_name, status) end
					end
				end
			elseif act == "blink" then
				return function(n)
					if n == nil then
						n = def
					end
					local f = filter_action(n);
					for _, source_name in pairs(f) do
						if source_name ~= cdwn_target and source_name ~= __CURRENT_SOURCE_NAME__ then
							Doblink(source_name, 100)
						end
					end
				end
					...
					for _, source_name in pairs(f) do
						if source_name ~= cdwn_target and source_name ~= __CURRENT_SOURCE_NAME__ then
							...
							for _, iter in ipairs(init_backup_list.Action) do
								if iter and iter.hide.name == source_name and iter.show and iter.show.name == source_name then
				...
				if p_item ~= nil then
					...
					elseif list ~= nil and list[1] == "<cond>" then
						...
						if cdwn_name == "cdwn" then
							...
							cdwn_do = get_do(cdwn_do,cdwn_main)
							--[[
								if cdwn_do == "blink" then
									cdwn_do = function(n)
										...
										for _, source_name in pairs(filter_action(n)) do
											if source_name ~= cdwn_name then Doblink(source_name, 100) end
										end
									end
								end
						...
						elseif cdwn_name == "cda" then
							if not init_list.Conion["cda"] then
								init_list.Conion["cda"] = {}
							end
							if not init_list.Conion["cda"][cdwn_act] then
								init_list.Conion["cda"][cdwn_act] = {}
							end
							cdwn_do = get_do(cdwn_do, cdwn_main)
							table.insert(init_list.Conion[cdwn_name][cdwn_act], {
								["do"] = cdwn_do; ["for"] = cdwn_main;["target"]=cdwn_target
							})
						end
					end
	...
	for k, iter in ipairs(init_list.Action) do
		...
		if iter.hide and not iter.hide.isHidden and iter.show and iter.show.isShown then -- preform hide
			local function initHide()
				...
				local target_source_name = iter.hide.name
				-- [[ ALL OR ALL_LIKE OR RAND OR RAND_LIKE ]]
					...
					elseif iter.hide.name == "rand" or (APP_ACTIONS:IsLikeAction(iter.hide.name) and APP_ACTIONS:CmdTar(iter.hide.name) == "rand") then -- {RAND, RAND_LIKE}
						if iter.hide.name == "rand" then
							local sources = get_all_current_sources_from_scene();
							if #sources > 0 then
								target_source_name = sources[math.random(1, #sources)]
								__CURRENT_SOURCE_NAME__ = target_source_name
								setSourceVisible(target_source_name, false)
							end
						else -- Select a random target that matches (tN)
							local tN = APP_ACTIONS:GetLikeAction(iter.hide.name);
							local sum = 1
						...
						__CURRENT_SOURCE_NAME__ = iter.hide.name
				...
				if iter.hide["repeat"] then
					...
					if iter.hide["repeat"].count >= iter.hide["repeat"].max then -- stop the hide from working;
						...
						if iter.hide["repeat"]["reset"] then -- reset
							...
							local function resetTimer()
								...
								if copyIter.hide["repeat"]["reset"]["type"] == "mis" then
									if (currTick - resetTick) >= copyIter.hide["repeat"]["reset"]["init"] then
										...
										-- Check for events;
										if init_list.Conion[target_source_name] and init_list.Conion[target_source_name]["cdwn"] and init_list.Conion[target_source_name]["cdwn"]["reset"] then
											for _, event in ipairs(init_list.Conion[target_source_name]["cdwn"]["reset"]) do
												event["do"]()
											end
										end
							...
							if init_list.Conion[target_source_name] and init_list.Conion[target_source_name]["cdwn"] and init_list.Conion[target_source_name]["cdwn"]["end"] then
								for _, event in ipairs(init_list.Conion[target_source_name]["cdwn"]["end"]) do
									event["do"]()
								end
							end
						end
				...
				-- Check for events(When)
				if init_list.Conion[target_source_name] and init_list.Conion[target_source_name]["cdwn"] and init_list.Conion[target_source_name]["cdwn"]["hide"] then
					for _, event in ipairs(init_list.Conion[target_source_name]["cdwn"]["hide"]) do
						event["do"]()
					end
				end
				-- Check for events(Any)
				if init_list.Conion["cda"] and init_list.Conion["cda"]["hide"] then
					for _, iter in pairs(init_list.Conion["cda"]["hide"]) do
						local target = filter_action(iter["target"])
						local current = filter_action(target_source_name)
						local is_ok = false
						for _, x in pairs(target) do
							for _, y in pairs(current) do
								if x == y then
									is_ok = true
									break
								end
		...
		elseif iter.hide and iter.hide.isHidden and iter.show and not iter.show.isShown then -- (Show)
			local function initShow()
				...
				local target_source_name = iter.show.name;
				-- [[ ALL OR ALL_LIKE OR RAND OR RAND_LIKE ]]
					...
					elseif iter.show.name == "rand" or (APP_ACTIONS:IsLikeAction(iter.show.name) and APP_ACTIONS:CmdTar(iter.show.name) == "rand") then -- {RAND, RAND_LIKE}
						if iter.show.name == "rand" then
							...
							if #sources > 0 then
								target_source_name = sources[math.random(1, #sources)]
								__CURRENT_SOURCE_NAME__ = target_source_name
								setSourceVisible(target_source_name, true)
							end
						else -- Select a random target that matches (tN)
							local tN = APP_ACTIONS:GetLikeAction(iter.show.name);
							local sum = 1
						...
						__CURRENT_SOURCE_NAME__ = iter.show.name
				...
				-- Check for events;
				if init_list.Conion[target_source_name] and init_list.Conion[target_source_name]["cdwn"] and init_list.Conion[target_source_name]["cdwn"]["show"] then
					for _, event in ipairs(init_list.Conion[target_source_name]["cdwn"]["show"]) do
						event["do"]()
					end
				end
				-- Check for events(Any)
				if init_list.Conion["cda"] and init_list.Conion["cda"]["show"] then
					for _, iter in pairs(init_list.Conion["cda"]["show"]) do
						local target = filter_action(iter["target"])
						local current = filter_action(target_source_name)
						local is_ok = false
						for _, x in pairs(target) do
