...
local animation_list = {
    ...
    {id = "mtl";name = "Move to left"},
    {id = "mtr";name = "Move to right"},
    {id = "mtt";name = "Move to top"},
    {id = "mtb";name = "Move to bttom"}
}
source = {}
source.id = "hns-filter-iixisii"
source.type = obs.OBS_SOURCE_TYPE_FILTER
source.output_flags = bit.bor(obs.OBS_SOURCE_VIDEO)
__TIMER_CALLBACK__ = nil;on_exit=false
...
source.destroy = function(_source_filter, data)
    if _source_filter.source ~= nil then
        ...
        if source_target == nil and __TIMER_CALLBACK__ ~= nil and not on_exit then
            obs.timer_remove(__TIMER_CALLBACK__)
            __TIMER_CALLBACK__ = nil
        end
    else -- this might crash!
        -- if __TIMER_CALLBACK__ ~= nil and not on_exit then
        --     obs.timer_remove(__TIMER_CALLBACK__)
        --     __TIMER_CALLBACK__ = nil
        -- end
    end
...
function welcomeIndex()
	return [[<center><h1 style = "color:#eee;padding:0;margin:0">HIDE & SHOW</h3><h5 style = 'color:#ddd'><i>Ver. 5.1.0 - Made by iixisii</i></h5></center>
		<center>
...
function animate(_source_filter, speed)
    return {
        left_to_right = function(limit)
            if __TIMER_CALLBACK__ ~= nil then -- add the animation to the queue
                return
            end
            local pos = obs.vec2()
            obs.obs_sceneitem_get_pos(_source_filter.sceneitem.data, pos)
            ...
            obs.timer_add(__TIMER_CALLBACK__, speed)
            return  {
                after = function(callback)
